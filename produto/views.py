...
def add_carrinho(request):
    ...
    if not request.session.get('carrinho'):
        request.session['carrinho'] = []
        request.session.save()
    # aqui está pegando tudo que foi enviado do formulário de produto.html e convertendo para um dicionário do python
    x = dict(request.POST)
    ...
    if not aprovado:
        messages.add_message(request, constants.ERROR, "Verifique a quantidade de adicionais")
